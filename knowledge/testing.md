...
| `test/tools/inline_gnd_reopen_check.mjs` | The inline-GND re-open/import inverse (`inline-gnd.js fromInlineGND`): an exported interchange document is read back into the register model, every inline `persName`/`orgName`/`bibl` lifted into `<standOff>` (deduplicated by GND, else by text, the GND moved onto an `<idno>`) and each mention rewrapped as `<name ref="#id">`; places are not recovered (one-way loss); the reading text is byte-identical through the round-trip; the interchange file is a FIXED POINT (`toInlineGND(fromInlineGND(file))` equals the file); and re-import on a register-model doc is a SAME-doc no-op (idempotent) | PASS |
| `test/tools/inline_gnd_export_wiring_check.mjs` | The editor's inline-GND export PATH (what the "Export inline-GND" button drives) through the EDITION layer: the bytes the button transforms ARE the canonical current document (`serialize(state) === state.doc.raw`, never a stale snapshot), `toInlineGND(state.doc).raw` re-parses, drops the register, leaves no `#`-mention, inlines the GND mention and unwraps the place, reading text byte-identical, a second pass is a SAME-doc no-op; an in-editor edit (`editCellCore`) is reflected in a re-export; and `inlineGndFilename` names the download `{base}_final.xml` (idempotent, fallback for a missing name) | PASS |
