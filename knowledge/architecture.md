...
| 3. UI controller | `js/editor/editor-app.js` | Shell wiring: loading via "Open TEI..." (File System Access API with a file-input fallback) plus an examples dropdown and a start screen carrying the three demo sources (Wenzelsbibel, ZBZ Hersch, SZD); page turning in the reading-pane header plus arrow keys; the inline cell action chooser (M2.6: click a cell for Edit / Note / Mark / link (word profile) / entity-jump / cancel in place, with a context label and a pure-cancel path that restores the span without a re-render); selection annotation (M2.8: a mouse selection inside a line opens an annotate popover, new-entity-per-type or link-existing, then reveals the index entry for authority ids); the M2.5 visibility layer (per-entity-type mention colours with tooltips, violet plus dashed outline for AI-proposed targets, a triple-coded note marker, a per-document legend strip that also hosts the mode hint); the OpenSeadragon facsimile with zone linking, auto-collapsed for documents without page images and toggleable otherwise; the live checks as a footer status chip with a detail popover; the Index-only right pane with the AI-suggest block; save and download; and the LLM on-ramp modal. All inline mutations run through one shared `applyDocFn` path. |
...
| `js/editor/index-panel.js` | Index-management UI (persons/places/orgs/works/events) with a per-entity authority-id field (register select + value + removable chips), a live authority-lookup affordance, an AI-proposed (violet) row state with a confirm action, and mention-count badges. Since M2.11 it renders inside the on-demand Index overlay (no permanent pane); row click jumps to the entity's first in-text mention, and day-to-day authority editing happens in the annotation popover in `editor-app.js` through the same `standoff.js` paths. Imports nothing project-internal. |
