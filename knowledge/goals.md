...
- (*) **M2.5** Annotation visibility layer (operator decision 2026-06-09, from the agent-driven live review): everything annotated must be visible in the reading text. A `cell.mention` projection in `edition.js` (walk the `<name ref>` ancestor), renderer classes wiring the existing but unused `.ed-w.mention` style with per-entity-type colours (the `--color-persName`/`--color-placeName`/`--color-orgName` token family, today unused in the editor) and tooltips carrying the entity name; a stronger triple-coded note marker; a legend strip in the reading-pane header. Token-only per design.md. **open**
- (*) **M2.6** Inline cell action chooser (operator decision 2026-06-09): clicking a cell offers Edit / Note / unclear / del / add / gap / Link (with an in-place entity picker) / cancel, reusing the proven `.ed-crit-pick` pattern, so nothing hides behind pre-toggled modes; the toolbar toggles stay as shortcuts and double-click is quick edit. **open**
