...
- **The panes are adjustable and remembered.** A draggable splitter sits between them (min 320px per pane, double-click resets to even, focus-and-arrow-keys resize it for keyboard use); a toggle at the right edge of the text-pane head folds the context pane away (or Ctrl/Cmd+\), after which the `@facs` click-sync simply has no visible target and raises nothing; below 900px the panes stack vertically. Editor-wide text zoom (A-/reset/A+) sits in the same header. Split position, collapsed state and the active context tab are remembered per document; the text zoom globally.
...
| Empty editor state (no welcome screen): the editor opens on its two-pane layout with a lean load prompt and the recent files in the text pane; document toolbar group and view controls hidden until load | Built (2026-06-10, supersedes the full-width welcome surface) |
| Document strip under the toolbar + Document context panel; neutral plaintext-draft banner; draft-recovery offer in the empty state | Built (2026-06-12) |
| Flat filterable selection popover with declared attribute fields (`attrField`); semantic-wrap dotted underline with attribute tooltip | Built (2026-06-12) |
| Example surfaces gated to local development hosts (landing section and button, Load... entries, `#example` deep link) | Built (2026-06-12) |
| Reading-text gutter: the edition's line label (@n) in a fixed mono channel with a continuous rule; keeps the source's numbering (empty cell for an unnumbered line), with a faint display-only positional fallback when a folio numbers nothing (never serialised); non-selectable | Built (2026-06-10, fallback 2026-06-12) |
