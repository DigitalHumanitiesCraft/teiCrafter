...
- Package F (specified 2026-06-12, queued): the declared manifest `indices` become the index panel's consumer (only declared indices render as sections, each with its own filter and add affordance; "Add index..." writes the declaration into a project folder's manifest, session-only for bare files; per-index registers feed the reconciliation), empty-project onboarding (opening or creating a project always switches to the Project panel; an empty project offers "New plaintext..."; a drop while a project is open offers adoption into the folder), the draft banner merging into the document strip (a Draft badge, the explanation in the Document panel), and the strip click returning the left pane to the reading view (a persisted XML-source view next to the Document panel read as "the panel shows XML" in operator use).
- The project manifest's remaining consumers beyond package F: the declared `views` as real authoring views (the F4 dual-reading switcher binds to the document's encoding, not to the declaration, which still has no consumer), and per-edition schema/Schematron validation from the `schema` field. The manifest format, its image-resolver and type-bound markup consumers, and "Open project folder" (M2.9) are built (see Key Decisions). Beyond folio-shaped editions: editing units and views for other edition types (dictionary entries, corpus sentences), as manifest-declared views over the same lossless core.
...
- Static typing (raised 2026-06-12, growing module count): full TypeScript would break the no-build-step architecture (source vs artifact, Pages from /docs, proofs importing browser modules directly). The recorded recommendation is JSDoc annotations checked by `tsc --noEmit` as a harness proof, starting at the engine seam; reversible, converts mechanically to TS if a bundler ever becomes necessary. Not yet commissioned.
