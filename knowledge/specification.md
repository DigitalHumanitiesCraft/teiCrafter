...
- **Licence boundary.** Real third-party TEI never committed; synthetic twins only. Every public example card must still load on the deployment, so a rights-restricted example (Wenzelsbibel, ZBZ Hersch) ships a committed synthetic structural twin that the example registry falls back to on a 404; the real object loads only where present (local).
