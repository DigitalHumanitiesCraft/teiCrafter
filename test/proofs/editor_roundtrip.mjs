 ...
 *
 * Run: node test/proofs/editor_roundtrip.mjs
 */
...
// The harness-localization half needs python + lxml (validate.py). On a bare
// checkout (CI js-gate has node only) it may be absent; the pure-JS round-trip
// and surgical-edit proof above stands on its own, so skip only this half with
// a visible note when python+lxml is unavailable.
const PYTHON = [process.env.TCR_PYTHON, "python", "python3", "py"]
  .filter(Boolean)
  .find((bin) => spawnSync(bin, ["-c", "import lxml"], { encoding: "utf-8" }).status === 0);

if (!PYTHON) {
  console.log("  --  harness localization skipped: python with lxml not available");
} else {
